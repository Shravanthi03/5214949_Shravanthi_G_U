...
char** bomberMan(int n, int r, char** grid, int* result_count) {
    *result_count = r;
    ...
    char** full_bomb_grid() {  //bomb grid helper fun
        char** fg = malloc(r * sizeof(char*));
        for (int i = 0; i < r; i++) {
            fg[i] = malloc((c + 1) * sizeof(char));
            for (int j = 0; j < c; j++) fg[i][j] = 'O';
            fg[i][c] = '\0';
        }
        return fg;
    }

    char** copy_grid(char** src) { //copy grid helper fun
        char** cp = malloc(r * sizeof(char*));
        for (int i = 0; i < r; i++) {
            cp[i] = malloc((c + 1) * sizeof(char));
            strcpy(cp[i], src[i]);
        }
        return cp;
    }

    char** detonate(char** src) { //explosion fun
        char** res = full_bomb_grid();
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                if (src[i][j] == 'O') {
                    res[i][j] = '.';
